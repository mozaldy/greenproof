// === File: apps/dashboard/src/app/dashboard/tasks/page.tsx ===
'use client'
...
import { ALL_TASKS, STATUS_META, TYPE_META, ALL_STATUSES } from '@/lib/mock-data' // Extracted mocks
  ...
  return (
    <div style={{ minHeight: '100vh', background: 'var(--bg-base)' }}>
      ...
      <PageHeader 
        ...
        rightContent={
          <>
            <Link href="/dashboard/simulator" className="flex items-center gap-2 px-3 py-2 rounded border text-xs font-semibold no-underline transition-colors" style={{ background: 'var(--bg-elevated)', borderColor: 'var(--border)', color: 'var(--accent)' }}>
              <Cpu size={13} /> Buat via Simulator
            </Link>
            <button className="flex items-center gap-2 px-4 py-2 rounded text-xs font-bold text-white border-none cursor-pointer" style={{ background: 'var(--primary)' }}>
      ...
      <div className="px-8 py-6">
        ...
        <div className="flex items-center gap-3 mb-4">
          ...
          <div className="flex items-center gap-1.5">
            ...
            {ALL_STATUSES.map((s) => (
              ...
              >
                {s === 'Semua' ? 'Semua' : STATUS_META[s]?.label ?? s}
              </button>
            ))}
          </div>
        </div>
        ...
        {/* ── Table ──────────────────────────────────────────────── */}
        <div className="rounded-lg overflow-hidden bg-[var(--bg-surface)] border border-[var(--border)]">
          <div className="grid px-4 py-2 bg-[var(--bg-elevated)] border-b border-[var(--border)] text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-wider" style={{ gridTemplateColumns: '80px 1fr 130px 120px 80px 100px 50px' }}>
