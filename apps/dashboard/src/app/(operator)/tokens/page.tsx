...
type FilterTab = 'Semua' | 'Dibuat (Mint)' | 'Diterima' | 'Ditahan'
...
export default function TokensPage() {
  ...
  const filteredTx = recentTransactions.filter(tx => {
    ...
    if (activeTab === 'Diterima') return tx.type === 'distributed'
    if (activeTab === 'Ditahan') return tx.type === 'locked' || tx.status === 'pending_verdict'
    if (activeTab === 'Dibuat (Mint)') return tx.type === 'mint' || tx.type === 'locked' // Karena lock sebenarnya adalah mint ke escrow
    return true
  })
  ...
  return (
    <div className="flex flex-col min-h-screen bg-[var(--bg-base)] pb-20">
      <PageHeader 
        title="Poin Reward & Dana Pengaman" 
        subtitle="Kelola anggaran, transparansi distribusi, dan kendali smart contract perusahaan Anda." 
        icon={Coins} 
      />

      <div className="p-6 sm:p-8 flex flex-col gap-8 max-w-[1400px] mx-auto w-full">
        
        {/* ── PANEL KEDAULATAN PERUSAHAAN (Sesuai Diagram Arsitektur) ── */}
        <div className="bg-gradient-to-r from-blue-900/40 to-emerald-900/20 border border-blue-500/30 rounded-2xl p-5 sm:p-6 shadow-sm flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
          {/* Efek Glow Latar */}
          <div className="absolute top-0 right-1/4 w-64 h-64 bg-blue-500/10 blur-[80px] rounded-full pointer-events-none" />
          
          <div className="flex items-center gap-5 relative z-10">
            <div className="w-14 h-14 rounded-full bg-blue-500/20 border border-blue-500/40 flex items-center justify-center text-blue-400 shrink-0">
              <ShieldCheck size={28} />
            </div>
            <div>
              ...
              <p className="text-[12px] text-blue-200/70 mt-1 font-medium">
                Kontrak eksklusif milik perusahaan. Terpisah sepenuhnya dari jaringan lain.
              </p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-4 relative z-10 bg-black/20 p-3 rounded-xl border border-white/5">
            ...
            <div>
              ...
              <div className="text-sm font-bold text-white">Dompet Memegang <span className="font-mono text-amber-400">TreasuryCap</span></div>
              <div className="text-[11px] text-slate-400 mt-0.5">Hanya Anda yang berhak mencetak poin (Mint).</div>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
          <StatCard title="Total Poin Dibuat (Minted)" value={summary.totalMinted} icon={Database} colorClass="text-blue-500" bgClass="bg-blue-500/10 border-blue-500/20" desc="Total suplai yang dicetak perusahaan" />
          <StatCard title="Terkirim ke Mitra" value={summary.distributed} icon={Gift} colorClass="text-emerald-500" bgClass="bg-emerald-500/10 border-emerald-500/20" desc={`Sudah masuk ke ${summary.distributedMitraCount} dompet mitra`} />
          <StatCard title="Dana Siaga (Escrow)" value={summary.lockedInEscrow} icon={Lock} colorClass="text-amber-500" bgClass="bg-amber-500/10 border-amber-500/20" desc={`${summary.escrowTaskCount} tugas perlu diverifikasi`} />
          <StatCard title="Rata-rata Distribusi" value={summary.avgDailyBurn} icon={ArrowUpRight} colorClass="text-purple-500" bgClass="bg-purple-500/10 border-purple-500/20" desc="Poin cair per hari (Bulan ini)" />
        </div>
        ...
        <div className="rounded-2xl border border-[var(--border)] bg-[var(--bg-surface)] p-6 sm:p-8 shadow-sm">
          <div className="flex flex-col sm:flex-row sm:items-center justify-between mb-8 gap-4">
            <div>
              <h3 className="font-sans text-base font-bold text-[var(--text-primary)]">Tren Aliran Poin (30 Hari Terakhir)</h3>
              <p className="text-xs text-[var(--text-muted)] mt-1 font-medium">Bandingkan jumlah poin yang cair vs tertahan.</p>
            </div>
            <div className="flex items-center gap-4 text-[10px] font-bold tracking-widest uppercase bg-[var(--bg-elevated)] px-4 py-2 rounded-full border border-[var(--border)]">
              <div className="flex items-center gap-2 text-emerald-500"><span className="w-2.5 h-2.5 rounded-full bg-emerald-500 shadow-[0_0_8px_#10b981]"></span> Terkirim</div>
          ...
          <div className="w-full relative group cursor-crosshair">
            <svg viewBox={`0 0 ${CHART_WIDTH} ${CHART_HEIGHT}`} className="w-full h-[220px] overflow-visible" preserveAspectRatio="none">
              <defs>
            ...
            {hoveredDay !== null && (
              <div className="absolute top-0 w-px h-full bg-[var(--border-strong)] z-0 pointer-events-none" style={{ left: `${(hoveredDay / (dailyHistory.length - 1)) * 100}%` }}>
                <div className="absolute top-0 -translate-x-1/2 -translate-y-full mb-2 bg-[var(--bg-surface)] border border-[var(--border-strong)] p-3 rounded-xl shadow-xl w-max pointer-events-none z-20">
                  <div className="text-[10px] font-bold text-[var(--text-muted)] border-b border-[var(--border)] pb-1 mb-2 text-center uppercase tracking-widest">
                    {dailyHistory[hoveredDay].date}
                  </div>
                  <div className="flex gap-4">
                    <div className="flex flex-col items-center">
                      <span className="font-mono text-[14px] font-black text-emerald-500">{dailyHistory[hoveredDay].distributed}</span>
                      <span className="text-[9px] uppercase tracking-wider text-[var(--text-muted)]">Terkirim</span>
                    </div>
                    <div className="w-px bg-[var(--border)]" />
                    <div className="flex flex-col items-center">
                      <span className="font-mono text-[14px] font-black text-amber-500">{dailyHistory[hoveredDay].locked}</span>
                      <span className="text-[9px] uppercase tracking-wider text-[var(--text-muted)]">Ditahan</span>
                    </div>
                  </div>
                </div>
                <div className="absolute -translate-x-1/2 w-2.5 h-2.5 rounded-full bg-emerald-500 border-2 border-[var(--bg-base)] shadow-[0_0_10px_#10b981]" style={{ top: `calc(100% - ${(dailyHistory[hoveredDay].distributed / maxVal) * 100}%)` }} />
                <div className="absolute -translate-x-1/2 w-2.5 h-2.5 rounded-full bg-amber-500 border-2 border-[var(--bg-base)] shadow-[0_0_10px_#f59e0b]" style={{ top: `calc(100% - ${(dailyHistory[hoveredDay].locked / maxVal) * 100}%)` }} />
              </div>
            )}
          </div>
        </div>

        {/* ── Tabel Transaksi: Disempurnakan untuk mencerminkan aliran SUI ── */}
        <div className="flex flex-col gap-4">
          <h3 className="font-sans text-sm font-bold text-[var(--text-primary)] pl-1">Data Transaksi Blockchain</h3>
          
          <div className="flex items-center gap-2 bg-[var(--bg-surface)] p-1.5 rounded-xl border border-[var(--border)] w-fit shadow-sm">
            {(['Semua', 'Dibuat (Mint)', 'Diterima', 'Ditahan'] as FilterTab[]).map(tab => (
              <button
                key={tab}
                onClick={() => setActiveTab(tab)}
                className={`px-5 py-2 rounded-lg text-[11px] font-bold tracking-wide uppercase transition-all cursor-pointer ${
                  activeTab === tab 
              ...
              <table className="w-full text-left border-collapse">
                <thead>
                  <tr className="bg-[var(--bg-elevated)] border-b border-[var(--border)]">
                    ...
                    <th className="p-4 text-[10px] font-bold text-[var(--text-muted)] uppercase tracking-widest whitespace-nowrap">Tx Hash Explorer</th>
                  </tr>
                </thead>
                <tbody className="divide-y divide-[var(--border-subtle)]">
                  {filteredTx.map(tx => {
                    ...
                    // Logika aliran sesuai arsitektur SUI di diagram
                    const source = isLocked ? "Dompet Perusahaan" : "Sistem Pengaman (Escrow)";
                    const target = isLocked ? "Sistem Pengaman" : (tx.recipient?.name || "Mitra");
                    const actionBadge = isLocked ? "DIBUAT & DITAHAN" : "DITRANSFER";
                      ...
                      <tr key={tx.id} className="hover:bg-[var(--bg-hover)] transition-colors group">
                        ...
                        <td className="p-4">
                          <div className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-[10px] font-bold uppercase tracking-wider border ${
                            ...
                            {isLocked ? <Database size={12} /> : <ArrowRightLeft size={12} strokeWidth={2.5} />}
                            {actionBadge}
                          </div>
                        </td>
                        <td className="p-4">
                        ...
                        <td className="p-4">
                          <div className="flex items-center gap-1.5 text-[11px] text-blue-500 font-mono cursor-pointer hover:underline opacity-80 group-hover:opacity-100 transition-opacity">
                            {tx.txHash.substring(0, 6)}...{tx.txHash.substring(tx.txHash.length - 4)} <ExternalLink size={12} />
                          </div>
                        </td>
                      </tr>
                    )
  ...
  return (
    <div className={`rounded-2xl p-5 border shadow-sm relative overflow-hidden transition-all hover:-translate-y-1 ${bgClass}`}>
      <div className="flex justify-between items-start mb-3">
        <div className={`p-2.5 rounded-lg bg-[var(--bg-base)] border border-[var(--border)] shadow-sm ${colorClass}`}>
          <Icon size={18} />
        </div>
      </div>
      <div>
        <h4 className={`text-[10px] font-bold uppercase tracking-widest mb-1.5 ${colorClass}`}>{title}</h4>
        <div className="font-mono text-3xl font-black text-[var(--text-primary)] mb-1.5 tracking-tight drop-shadow-sm">
          {value.toLocaleString()}
        </div>
        <p className="text-[11px] text-[var(--text-secondary)] font-medium leading-snug">{desc}</p>
      </div>
